...
const PharmacyDoctorDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loader = token ? api.getDoctorsDirectoryForPharmacy(token) : api.getDoctorsDirectory();
  ...
  const verifyDoctor = async () => {
    ...
    try {
      ...
      const updated = await api.pharmacyVerifyDoctorLicense(token, doctor.id, { verified: true });
  ...
  const unverifyDoctor = async () => {
    ...
    try {
      setUpdating(true);
      setActionMessage(null);
      const updated = await api.pharmacyVerifyDoctorLicense(token, doctor.id, { verified: false });
      setDoctor((prev) => (prev ? { ...prev, ...updated } : updated));
    } catch (error) {
  ...
  const approveDoctor = async () => {
    ...
    try {
      setApproving(true);
      await api.pharmacyApproveDoctorAccount(token, doctor.id);
      const rows = await api.getDoctorsDirectoryForPharmacy(token);
  ...
  const unapproveDoctor = async () => {
    ...
    try {
      setApproving(true);
      setActionMessage(null);
      await api.pharmacyUnapproveDoctorAccount(token, doctor.id);
      const rows = await api.getDoctorsDirectoryForPharmacy(token);
      const refreshed = rows.find((row) => row.id === doctor.id) ?? doctor;
      setDoctor(refreshed);
    } catch (error) {
      ...
      <IonHeader>
        <IonToolbar>
          <IonButtons slot="start">
            <IonBackButton defaultHref="/pharmacy/doctors" />
          </IonButtons>
                ...
                <IonItem lines="none">
                  ...
                  <div slot="end" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <IonToggle
                      ...
                      disabled={!canVerify || updating || approving || user?.id === doctor.id}
                      onIonChange={(event) => {
...
export default PharmacyDoctorDetailPage;
