...
const AdminHospitalsPage: React.FC = () => {
  ...
  const renderHospitalCard = (hospital: ApiPharmacy) => (
    <IonCard
      key={hospital.id}
      ...
      <IonCardContent>
        <div style={{ display: 'block' }}>
          <div>
            ...
            <h3>{hospital.name}</h3>
            <p>{hospital.address || 'Adresse N/D'}</p>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '6px' }}>
          ...
          <IonBadge color={hospital.license_verified ? 'success' : 'warning'}>
            {hospital.license_verified ? 'Licence OK' : 'Licence attente'}
          </IonBadge>
          <IonBadge color={hospital.account_can_verify_accounts ? 'tertiary' : 'medium'}>
            {hospital.account_can_verify_accounts ? 'Delegue verif' : 'Sans delegation'}
          </IonBadge>
        </div>
      </IonCardContent>
    </IonCard>
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          <IonTitle>Admin · Hopitaux</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent className="ion-padding app-content">
        ...
        <IonCard className="surface-card">
          <IonCardContent>
            ...
            <div>{priorityRows.map((hospital) => renderHospitalCard(hospital))}</div>
            ...
            <div>{compliantRows.map((hospital) => renderHospitalCard(hospital))}</div>
          </IonCardContent>
        </IonCard>
      </IonContent>
    </IonPage>
